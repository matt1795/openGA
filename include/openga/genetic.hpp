...
namespace OpenGA {
    ...
    class Genetic {
      ...
      public:
        ...
        std::function<double(const thisChromosomeType&)>
            calculate_SO_total_fitness;
        std::function<std::vector<double>(thisChromosomeType&)>
            calculate_MO_objectives;
        std::function<std::vector<double>(const std::vector<double>&)>
            distribution_objective_reductions;
        std::function<void(GeneType&, const std::function<double(void)>& rnd01)>
            init_genes;
        std::function<bool(const GeneType&, MiddleCostType&)> eval_solution;
        std::function<bool(const GeneType&, MiddleCostType&,
                           const thisGenerationType&)>
            eval_solution_IGA;
        std::function<GeneType(const GeneType&,
                               const std::function<double(void)>& rnd01,
                               double shrink_scale)>
            mutate;
        std::function<GeneType(const GeneType&, const GeneType&,
                               const std::function<double(void)>& rnd01)>
            crossover;
        std::function<void(int, const thisGenerationType&, const GeneType&)>
            SO_report_generation;
        std::function<void(int, const thisGenerationType&,
                           const std::vector<unsigned int>&)>
            MO_report_generation;
        std::function<void(void)> custom_refresh;
        std::function<double(int, const std::function<double(void)>& rnd01)>
            get_shrink_scale;
        ...
        Genetic()
            : unif_dist(0.0, 1.0), N_robj(0), problem_mode(GA_MODE::SOGA),
              population(50), crossover_fraction(0.7), mutation_rate(0.1),
              verbose(false), generation_step(-1), elite_count(5),
              generation_max(100), tol_stall_average(1e-4),
              average_stall_max(10), tol_stall_best(1e-6), best_stall_max(10),
              reference_vector_divisions(0), enable_reference_vectors(true),
              multi_threading(true), dynamic_threading(true),
              N_threads(std::thread::hardware_concurrency()),
              user_request_stop(false), idle_delay_us(1000),
              calculate_IGA_total_fitness(nullptr),
              calculate_SO_total_fitness(nullptr),
              calculate_MO_objectives(nullptr),
              distribution_objective_reductions(nullptr), init_genes(nullptr),
              eval_solution(nullptr), eval_solution_IGA(nullptr),
              mutate(nullptr), crossover(nullptr),
              SO_report_generation(nullptr), MO_report_generation(nullptr),
              custom_refresh(nullptr), get_shrink_scale(default_shrink_scale) {
        ...
        void calculate_N_robj(const thisGenerationType& g) {
            if (!g.chromosomes.size())
                throw std::runtime_error(
                    "Code should not reach here. A87946516564");
            ...
            if (!N_robj)
                throw std::runtime_error(
                    "Number of the reduced objective is zero");
        }
        ...
        void solve_init() {
            ...
            if (!is_single_objective()) {
                ...
                if (!reference_vector_divisions) {
                    ...
                    if (N_robj == 1)
                        throw std::runtime_error(
                            "The length of objective vector is 1 in a "
      ...
      protected:
        static double
        default_shrink_scale(int n_generation,
                             const std::function<double(void)>& rnd01) {
            double scale =
                (n_generation <= 5 ? 1.0 : 1.0 / sqrt(n_generation - 5 + 1));
            if (rnd01() < 0.4)
                scale *= scale;
            else if (rnd01() < 0.1)
                scale = 1.0;
        ...
        void report_generation(const thisGenerationType& new_generation) {
            if (is_single_objective()) { // SO (including IGA)
                SO_report_generation(
                    ...
                    new_generation
                        .chromosomes[new_generation.best_chromosome_index]
                        .genes);
            } else {
            ...
            if (!is_interactive()) { // add all members
                ...
                for (int i = 0; i < elite_count; i++)
                    new_generation.chromosomes.push_back(
                        last_generation
                            .chromosomes[last_generation.sorted_indices[i]]);
            }
        }
        ...
        void finalize_generation(thisGenerationType& new_generation) {
            ...
            if (is_single_objective()) {
                ...
                for (unsigned int i = 0; i < new_generation.chromosomes.size();
                     i++) {
                    double current_cost =
                        new_generation.chromosomes[i].total_cost;
                    sum += current_cost;
        ...
        void check_settings() {
            if (is_interactive()) {
                if (calculate_IGA_total_fitness == nullptr)
                    throw std::runtime_error("calculate_IGA_total_fitness is "
                                             "null in interactive mode!");
                if (calculate_SO_total_fitness != nullptr)
                    throw std::runtime_error(
                        "calculate_SO_total_fitness is not null in "
                        "interactive mode!");
                if (calculate_MO_objectives != nullptr)
                    throw std::runtime_error("calculate_MO_objectives is not "
                                             "null in interactive mode!");
                if (distribution_objective_reductions != nullptr)
                    throw std::runtime_error(
                        "distribution_objective_reductions is not "
                        "null in interactive mode!");
                if (MO_report_generation != nullptr)
                    throw std::runtime_error("MO_report_generation is not null "
                                             "in interactive mode!");
                if (eval_solution_IGA == nullptr)
                    throw std::runtime_error(
                        "eval_solution_IGA is null in interactive mode!");
                if (eval_solution != nullptr)
                    throw std::runtime_error(
                        "eval_solution is not null in interactive "
                        "mode (use eval_solution_IGA instead)!");
            } else {
                if (calculate_IGA_total_fitness != nullptr)
                    throw std::runtime_error(
                        "calculate_IGA_total_fitness is not null "
                        "in non-interactive mode!");
                if (eval_solution_IGA != nullptr)
                    throw std::runtime_error("eval_solution_IGA is not null in "
                                             "non-interactive mode!");
                if (eval_solution == nullptr)
                    throw std::runtime_error("eval_solution is null!");
                if (is_single_objective()) {
                    if (calculate_SO_total_fitness == nullptr)
                        throw std::runtime_error(
                            "calculate_SO_total_fitness is null in "
                            "single objective mode!");
                    if (calculate_MO_objectives != nullptr)
                        throw std::runtime_error(
                            "calculate_MO_objectives is not null "
                            "in single objective mode!");
                    if (distribution_objective_reductions != nullptr)
                        throw std::runtime_error(
                            "distribution_objective_reductions is "
                            "not null in single objective mode!");
                    if (MO_report_generation != nullptr)
                        throw std::runtime_error(
                            "MO_report_generation is not null in "
                            "single objective mode!");
                } else {
                    if (calculate_SO_total_fitness != nullptr)
                        throw std::runtime_error(
                            "calculate_SO_total_fitness is no null "
                            "in multi-objective mode!");
                    if (calculate_MO_objectives == nullptr)
                        throw std::runtime_error(
                            "calculate_MO_objectives is null in "
                            "multi-objective mode!");
                    // if(distribution_objective_reductions==nullptr)
                    // 	throw runtime_error("distribution_objective_reductions
                    // is null in multi-objective mode!");
                    if (MO_report_generation == nullptr)
                        throw std::runtime_error(
                            "MO_report_generation is null in "
                            "multi-objective mode!");
                }
            }
            ...
            if (is_single_objective()) { // SO (including IGA)
                if (SO_report_generation == nullptr)
                    throw std::runtime_error(
                        "SO_report_generation is not adjusted "
                        "while problem mode is single-objective");
                if (MO_report_generation != nullptr)
                    throw std::runtime_error(
                        "MO_report_generation is adjusted while "
                        "problem mode is single-objective");
            } else {
                if (SO_report_generation != nullptr)
                    throw std::runtime_error(
                        "SO_report_generation is adjusted while "
                        "problem mode is multi-objective");
                if (MO_report_generation == nullptr)
                    throw std::runtime_error(
                        "MO_report_generation is not adjusted "
                        "while problem mode is multi-objective");
            }
        }

        void select_population(const thisGenerationType& g,
        ...
        void update_ideal_objectives(const thisGenerationType& g, bool reset) {
            ...
            if (reset) {
                if (distribution_objective_reductions)
                    ideal_objectives = distribution_objective_reductions(
                        g.chromosomes[0].objectives);
                else
            ...
            for (thisChromosomeType x : g.chromosomes) {
                std::vector<double> obj_reduced;
                if (distribution_objective_reductions)
                    obj_reduced =
                        distribution_objective_reductions(x.objectives);
                else
        ...
        void select_population_MO(const thisGenerationType& g,
            ...
            const unsigned int N_chromosomes =
                (unsigned int)g.chromosomes.size();
            Matrix zb_objectives(N_chromosomes, N_robj);
            for (unsigned int i = 0; i < N_chromosomes; i++) {
                std::vector<double> robj_x;
                if (distribution_objective_reductions)
                    robj_x = distribution_objective_reductions(
            ...
            if (reference_vectors.empty()) {
                ...
                reference_vectors = generate_referenceVectors(
                    obj_dept, reference_vector_divisions);
            }
            ...
            associate_to_references(g, norm_objectives, associated_ref_vector,
                                    distance_ref_vector, niche_count,
                                    distances);
            ...
            while (g2.chromosomes.size() + to_add.size() < population) {
                if (!enable_reference_vectors) { // disabling reference points
                    ...
                    unsigned int to_add_index =
                        (unsigned int)std::floor(msz * random01());
                    if (to_add_index >= msz)
                        to_add_index = 0;
                ...
                if (niche_count[min_niche_index] == 0) {
                    double min_val =
                        distances(min_vec_neighbors[0], min_niche_index);
                ...
                unsigned int to_add_index =
                    min_vec_neighbors[next_member_index];
        ...
        void associate_to_references(
            ...
            std::vector<unsigned int>& associated_ref_vector,
            std::vector<double>& distance_ref_vector,
            std::vector<unsigned int>& niche_count, Matrix& distances) {
            ...
            for (unsigned int i = 0; i < N_x; i++) {
                ...
                unsigned int dist_min_index =
                    0; // to avoid uninitialization warning
        ...
        void build_hyperplane_intercepts(std::vector<double>& xinv) {
            /*
                    ...
                    y=(1.0)./x
            */
            assert(extreme_objectives.get_n_rows() ==
                       extreme_objectives.get_n_cols() &&
        ...
        unsigned int index_of_min(const std::vector<T>& v) {
            return (unsigned int)(std::distance(
                v.begin(), std::min_element(v.begin(), v.end())));
        }
        ...
        void scalarize_objectives(const Matrix& zb_objectives) {
            ...
            if (scalarized_objectives_min.empty()) {
                ...
                scalarized_objectives_min.assign(
                    N_objectives, std::numeric_limits<double>::infinity());
            }
        ...
        void quicksort_indices_SO(std::vector<int>& array_indices,
            ...
            if (left < right) {
                ...
                while (l < r) {
                    while (
                        (gen.chromosomes[array_indices[l]].total_cost <= x) &&
                        (l < right))
                        l++;
        ...
        void rank_population_MO(thisGenerationType& gen) {
            ...
            do {
                next_front.clear();
                std::vector<unsigned int>& last_front =
                    gen.fronts[gen.fronts.size() - 1];
                for (unsigned int i : last_front)
        ...
        bool dominates(const thisChromosomeType& a,
                       const thisChromosomeType& b) {
        ...
        generate_integerReferenceVectors(int dept, int N_division) {
            ...
            for (int i = 0; i <= N_division; i++) {
                ...
                tail =
                    generate_integerReferenceVectors(dept - 1, N_division - i);
        ...
        void init_population_range(thisGenerationType* p_generation0,
        ...
        void init_population_single(thisGenerationType* p_generation0,
                                    int index, unsigned int* attemps,
                                    int* active_thread) {
            bool accepted = false;
            while (!accepted) {
                ...
                if (is_interactive()) {
                    if (eval_solution_IGA(X.genes, X.middle_costs,
                                          *p_generation0)) {
        ...
        void idle() {
            ...
            if (idle_delay_us > 0)
                std::this_thread::sleep_for(
                    std::chrono::microseconds(idle_delay_us));
        }
        ...
        void init_population(thisGenerationType& generation0) {
            ...
            if (!multi_threading || N_threads == 1 || is_interactive()) {
                int dummy;
                for (unsigned int i = 0; i < population && !user_request_stop;
                     i++)
                    init_population_single(&generation0, -1, &total_attempts,
                                           &dummy);
            } else {
                for (unsigned int i = 0; i < population; i++)
                    generation0.chromosomes.push_back(thisChromosomeType());
                std::vector<int> active_threads; // vector<bool> is broken
                ...
                if (dynamic_threading) {
                    ...
                    for (int i = 0; i < N_threads; i++) {
                        ...
                        if (i + 1 == N_threads) // last chunk
                            ...
                            x_index_end =
                                std::min(x_index_end, int(population) - 1);
        ...
        int select_parent(const thisGenerationType& g) {
            ...
            double r = random01();
            int position = 0;
            while (position < N_max &&
                   g.selection_chance_cumulative[position] < r)
                position++;
            return position;
        }
        ...
        void crossover_and_mutation_range(thisGenerationType* p_new_generation,
            ...
            for (int i = x_index_begin; i <= x_index_end; i++)
                crossover_and_mutation_single(p_new_generation,
                                              pop_previous_size, i, &dummy);
            *active_thread = 0; // false
        }
            ...
            while (!successful) {
                ...
                if (is_interactive()) {
                    ...
                    if (eval_solution(X.genes, X.middle_costs)) {
                        if (index >= 0)
                            p_new_generation
                                ->chromosomes[pop_previous_size + index] = X;
                        else
                            p_new_generation->chromosomes.push_back(X);
                        successful = true;
                    }
        ...
        void crossover_and_mutation(thisGenerationType& new_generation) {
            ...
            if (generation_step <= 0)
                return;
            unsigned int N_add = (unsigned int)(std::round(
                double(population) * (crossover_fraction)));
            unsigned int pop_previous_size =
                (unsigned int)new_generation.chromosomes.size();
            if (is_interactive()) {
                if (N_add + elite_count != population)
                    throw std::runtime_error(
                        "In IGA mode, elite fraction + crossover "
                        "fraction must be equal to 1.0 !");
            }

            if (!multi_threading || N_threads == 1 || is_interactive()) {
                int dummy;
                for (unsigned int i = 0; i < N_add && !user_request_stop; i++)
                    crossover_and_mutation_single(
                        &new_generation, pop_previous_size, -1, &dummy);
            } else {
        ...
        StopReason stop_critera() {
            ...
            if (is_single_objective()) {
                ...
                if (std::abs(g1.best_total_cost - g2.best_total_cost) <
                    tol_stall_best)
                    best_stall_count++;
                else
                    best_stall_count = 0;
                if (std::abs(g1.average_cost - g2.average_cost) <
                    tol_stall_average)
                    average_stall_count++;
                else
                    average_stall_count = 0;
